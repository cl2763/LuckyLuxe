...
/* ══ J-50 第二款(店主 07l §三 立,2026-09-14)· **算出来的底数变小,也要归因** ══
 *
 * J-50 原本只管棘轮。07l 撞出了它的镜像:
 *   「该跑 / 该扫 / 该查」这类**算出来的**底数,变小时**看起来像好事**(名单短了=没那么多要做了)。
 *
 * 案由(09-14 现撞):门关档「该跑哪些」按「源码里出现顾客侧取/用 token 的形态」算,
 * 而那串形态**只认演示/邮箱登录,不认微信正门**。于是裁 #60 让我们一套套改成走正门 ——
 * **每改好一套,那一套就从「该跑」名单里掉出去一个**,覆盖数一路变小,判据一路绿。
 *
 * 所以归因从三选一扩成**四选一**,新加的第 ④ 项最阴,因为它
 * **恰好发生在我们做对事情的时候**:修得越多,扫得越少。
 *   ① 真修好了            —— 被测对象真的没了
 *   ② 扫描面缩了          —— 少扫了几个文件
 *   ③ 判据放松了          —— 门槛调低了
 *   ④ **被测对象换了形态,判据没跟上** ← 07l 新增
 *   ⑤ **搬到另一档了**    ← 07l 新增(裁 #91 把三把小程序刀移出主档就是这一种)
 * 后四种数字都变好看,问题一点没少。写了归因的好处是:**红的时候看得出是哪一种。** */
const ATTR_TAG = /棘轮归因\s*[::]\s*(真修好了|扫描面缩了|判据放松了|形态换了判据没跟上|搬到另一档了)\s*([^\n]*)/
const ATTR_OK = '真修好了'
