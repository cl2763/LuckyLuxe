...
async function api(tenantId, path, options = {}) {
  const res = await fetch(`${BASE}${path}`, {
    ...options,
    headers: { 'content-type': 'application/json', 'x-demo-seed': 'seed-bigdemo', authorization: `Bearer ${TOKEN}`, 'x-admin-tenant-id': tenantId, ...(options.headers || {}) }
  })
...
async function pub(path, options = {}) {
  const res = await fetch(`${BASE}${path}`, { ...options, headers: { 'content-type': 'application/json', 'x-demo-seed': 'seed-bigdemo', ...(options.headers || {}) } })
  const text = await res.text()
