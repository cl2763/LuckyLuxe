...
const BLOCKS = [
  { key: 'light', head: ':root{' },
  { key: 'system-dark', head: ':root:not([data-theme="light"]){' },
  { key: 'theme-dark', head: ':root[data-theme="dark"]{' },
]

/** 从一段 CSS 文本里抽出某个选择器块的令牌表。
 *  故意写得笨:找到 `选择器{`,一路读到配对的 `}`,再按 `--名:值;` 拆。
 *  不用正则一把梭 —— 正则读嵌套(`@media` 里那一段)会读错,而读错的判据比没有判据更坏。 */
function tokensOf(css, head) {
...
function compare(label, path, { required = true } = {}) {
  ...
  for (const b of BLOCKS) {
    ...
    const got = tokensOf(css, b.head)
    if (!want) { say(false, `合同图里找不到 ${b.head} 这一段`, '基准坏了,先修图'); continue }
    if (!got) { say(false, `${label} 缺 \`${b.head}\` 整段`, `合同图里有 ${want.size} 条`); continue }
