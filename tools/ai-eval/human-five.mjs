...
const lines = [`# ⑤ 像人${TALKS.length}通 —— 整段对话原文(店主打分用)`, '',
  ...
  /* 🔴 这一格是「本跑缺件」——**每跑前必须现验一遍再写**。
     v6 头一版照抄旧文写了「D166 仍未修」,而同一跑的原文里它明明是好的:
     缺件清单也是判据的一种,**照抄 = 没验**(同族:判据看代码不看注释)。 */
  '> ✅ **D166(问时长取会话里点过名的项目)现测已修**:点过名 → 答那个项目的 `base_duration_min`;',
  '> 没点名 → 按采集已定的**大类**给真实区间(不跨美甲美睫)。两处证据就在通六轮 4 与通一轮 4。', '',
  '> ✅ **v6 这一跑新修的三条,请重点读**:',
