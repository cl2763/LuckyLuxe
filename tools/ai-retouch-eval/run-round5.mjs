...
import { promptR5 } from './round5-prompts.mjs'
...
const DIR_A = join(ROOT, 'A_口令重组'), DIR_O = join(ROOT, '原图')
mkdirSync(DIR_A, { recursive: true }); mkdirSync(DIR_O, { recursive: true })

console.log(`第五轮 腿A:${CASES.length} 次 ≈ ¥${(CASES.length * YUAN).toFixed(2)}`)
const log = []
let spend = 0
for (const [i, [f, tone, why]] of CASES.entries()) {
  ...
  const 人话 = `${num}_模型四块口令_${桶名[b]}_${甲名[tone]}.jpg`
...
writeFileSync(join(ROOT, '_跑批日志_腿A.json'), JSON.stringify({
