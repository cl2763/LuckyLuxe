/* P1 手部探针(12d补 §三-2 + §七-4)—— 桶 × 甲面色调 2×2 覆盖,6 次
...
import { handPromptFor, bucketOfSkin } from './round4-prompts.mjs'
...
console.log(`P1 手部探针:${jobs.length} 次 ≈ ¥${(jobs.length * YUAN).toFixed(2)}`)
const log = []
let spend = 0
for (const [i, j] of jobs.entries()) {
  const src = join(IN_DIR, j.f), d0 = dimsOf(src), size = fitSize(d0)
  const dst = join(OUT_DIR, `H_${j.b}${j.tone}_${j.f}`)
  ...
  for (let a = 1; a <= 2; a += 1) {
    ...
    try {
      const r = await fetch('https://ark.cn-beijing.volces.com/api/v3/images/generations', {
        ...
        body: JSON.stringify({ model: MODEL, prompt: handPromptFor(j.b, j.tone), image: `data:image/jpeg;base64,${b64}`, size, watermark: false, response_format: 'url' }),
...
writeFileSync(join(OUT_DIR, '_跑批日志.json'), JSON.stringify({
  ...
  口令: Object.fromEntries([['A深', handPromptFor('A', '深')], ['A浅', handPromptFor('A', '浅')], ['B深', handPromptFor('B', '深')], ['B浅', handPromptFor('B', '浅')]]),
  明细: log,
}, null, 2))
