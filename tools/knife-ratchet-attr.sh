...
sed -i '' 's/#\[0-9a-fA-F\]{3,8}\\b/#[0-9a-fA-F]{6,8}\\b/' tools/pre-regression.sh
...
sed -i '' "s/\"\$HARDCOLOR\" -le 31/\"\$HARDCOLOR\" -le $AFTER/; s/HARDCOLOR > 31/HARDCOLOR > $AFTER/; s/≤ 31/≤ $AFTER/" tools/pre-regression.sh
...
sed -i '' 's|! -path "\*/styles/fraunces-digits.wxss"|! -path "*/styles/fraunces-digits.wxss" ! -path "*/pages/*"|' tools/pre-regression.sh
...
sed -i '' "s/\"\$MPCOLOR\" -le 2356/\"\$MPCOLOR\" -le $MA/; s/MPCOLOR > 2356/MPCOLOR > $MA/; s/≤ 2356/≤ $MA/" tools/pre-regression.sh
