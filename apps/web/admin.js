...
const ADMIN_BUILD = '20260922a-placeholder'
...
/* D156(店主 09-08 裁):顶栏第二行显**当前店名**。
   名字只有一个来源 —— 后端 `/admin/auth/me` 的 `storeName`(= `stores.name`,
   商家在门店设置里改的就是它)。前端**不拼不猜**,也不回落到商户名/显示名/写死的店名。
   三态:
   · 还没登录 → 显示控制台那句(那是登录前的标签,不是别人家的店名);
   · 登录了、后端给了名字 → 显示它;
   · 登录了、后端没给 → 「—」。**绝不留着上一家店的名字**(换店时那就是「看不出自己在哪」)。 */
function renderBrandSubtitle() {
  if (!ownerBearer()) {
    els.adminBrandSubtitle.textContent = isOwnerRole() ? t('ownerConsole') : t('staffConsole')
    return
  }
  els.adminBrandSubtitle.textContent = owner.storeName || '—'
}
...
async function loadAll() {
  ...
  owner.storeName = meData.admin?.storeName || ''   // D156:顶栏那行的唯一来源,取不到就空着让它出「—」
  renderBrandSubtitle()
...
function ownerLogout() {
  ...
  owner.role = 'owner'
  owner.token = ''
  owner.storeName = ''   // D156:退出就把店名清掉 —— 留着它,换个人登进来看到的是上一家店
...
function setLocked(locked) {
  /* D156:一锁回登录页就把店名清掉并重画那一行 ——
     401 掉线、切账号、退出,走的都是这里;留着旧店名就是「看不出自己在哪」的那个病。 */
  if (locked) { owner.storeName = ''; renderBrandSubtitle() }
...
/* 模块① 上架服务列表已搬出到 /web/service-catalog.js(公约②,2026-08-25 S13①) */
  ...
  els.serviceEditor.innerHTML = `
    <form class="service-editor-card card" id="serviceEditorForm">
      ...
      <div class="form-grid">
        ...
        ${/* 🔴 分类唯一真相律②(店主 2026-08-25):**编辑项目时只选大类,不许有第二个分类输入框**。
              这里原来是一个自由文本「分类」框(法式系列/日式款…),与大类各说各的 ——
              顾客端就是被它搞成「8 个分组各 1 项」的。现在只剩大类下拉;
              自由文本列已退役为派生字段(读时 join 大类字典,写时不接受)。 */''}
        <label><span>${owner.lang === 'zh' ? '大类(唯一分类口)' : 'Category'}</span><select name="categoryId">
          <option value="">${owner.lang === 'zh' ? '— 请选一个大类 —' : '— pick one —'}</option>
          ${pricingState.categories.map((c) => `<option value="${c.id}" ${c.id === service.categoryId ? 'selected' : ''}>${escapeHtml(c.name)}</option>`).join('')}
        </select></label>
...
async function savePackage(kind, existing) {
  ...
  let legacyGroupBad = false
  if (isTimes) {
    const cats = (await request('/admin/pricing/categories').catch(() => ({ categories: [] }))).categories || []
    groupOptions = groupOptions.concat(cats.map((c) => [c.name, c.name]))
    const cur = existing ? String(existing.projectGroup || '') : ''
    if (cur && !cats.some((c) => c.name === cur)) {
      legacyGroupBad = true
      groupOptions.unshift([cur, `⚠ ${cur}(分类已不存在,请改选)`])
    }
  }
  openFormModal({
