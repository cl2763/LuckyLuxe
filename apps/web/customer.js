...
/* 🔴 裁 #105(夜13 §六)· 中英文案表**整块摘进** `./customer-copy.js`(315 行)。
   它是**工厂**不是纯数据 —— 表里有 `${moneyY(50)}` 这种对本文件函数的调用,
   当纯数据摘出去,加载那一刻就 ReferenceError、整页白屏(摘的时候当场量出来的)。
   取不到就退到一个空壳 —— **顶层不许因为加载顺序变化而炸**(判据 ㉜ 咬过一次)。 */
const copy = typeof window.CustomerCopyFactory === 'function'
