...
/* 🔴「那就 10:30」是**挑时段**——既是确认也指定了时间。
   05n 新尺当场咬出来:机器一个都不认,三跑到底率 0/12(老尺 7/12)。
   尺子改对了才照出这个洞 —— 这就是「尺子两列并排」的价值。 */
for (const yes of ['那就 15:00', '那就15:00', '那就 10:30 吧', '就 15:00']) {
  check(`①h2 挑时段算确认:「${yes}」`, looksConfirm(yes))
}
/* 反向守:不带钟点的「那就…」是放弃或指代,不许当确认 */
for (const no of ['那就算了', '那就这个吧', '那要多久']) {
  check(`①h3 不带钟点的「那就…」不算确认:「${no}」`, !looksConfirm(no))
}
   ...
   凡是「一次抛出 ≥5 个编号项」的回复,不管美甲美睫、不管措辞,一律算表。 */
{
  ...
  const t2 = r2?.reply?.data?.answerZh || ''
  /* 🔴 项目**还没定**那一刻:`quoteMissingQuestions` 回空,头一版就在这兜底甩整张表
     —— 200 句里还剩 7 句出表(裁定要的是 0)。项目没定就先问项目。 */
  const noSvc = await sayTo(tid, `nosvc-${RUN}`, '有哪位技师比较擅长法式?')
  const nt = noSvc?.reply?.data?.answerZh || ''
  check('⑰a2 🔴 项目未定时也不许兜底甩整张表', !isForm(nt), nt.slice(0, 50))
  check('⑰a3 项目未定时问的就是项目', /美甲还是美睫|做什么项目|哪个项目/.test(nt) || nt.length <= 60, nt.slice(0, 50))
