...
`)
try {
  /* 小记图片(店主 08-30f 二部三):图随小记只追加;走图库同款 base64 通道;默认私密不上墙。
     ⚠️ 迁移顺序学费(全新库现崩抓的):这条 ALTER 必须排在 service_notes 的 CREATE 之后 ——
     放进前面的迁移簇会在全新库上「表还没建先 ALTER」直接起不来。 */
  db.exec('ALTER TABLE service_notes ADD COLUMN images_json TEXT')
} catch (error) {
  if (!String(error.message || '').includes('duplicate column')) throw error
}
