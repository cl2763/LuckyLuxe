...
const ALLOW = [
  ...
  /* D190 过渡态(店主 07g 段三 + 夜10 已放行,2026-09-13):
     网页顾客端**撤掉了两个点不动的登录入口**(邮箱 403 / Google 404),换成一张引导卡。
     这两句确实是「指路换端」——**但它不是死口,恰恰相反**:
     死口是「界面上有入口,点下去不通」;这里是**把不通的入口撤掉,如实告诉顾客路在哪**。
     店主定的口径:**没有入口不丢人,有入口但点不动才丢人。**
     ⚠️ 豁免有效期:网页目标态(手机号 + 短信验证码)做完之后,这两条要一起删。 */
  /* 🔴 夜13 §六:这一条**搬家了** —— 裁 #105 把顾客端中英文案表整块摘进 `customer-copy.js`,
     白名单锚在 `file` 上,文件一换它当场红。这不是缺陷,是**白名单锚在位置上**的代价
     (J-58③ 同族:锚不许选在本次改动会抹掉的东西上)。这里跟着搬,marker 那句话一个字没改。 */
  { file: 'apps/web/customer-copy.js', marker: '顾客端的下单、卡包、订单在小程序里', max: 1,
    reason: 'D190 过渡态引导卡(中文):网页那两个登录入口生产上 403/404,已撤;这句是如实指路,不是死口。目标态(短信码)做完即删' }
  /* ⚠️ 英文那句**不需要豁免**:这把刀按「小程序」三个字认指路句,英文串根本不会被它认出来。
     我一度也给它加了一条 —— 被「白名单零僵尸条目」那条反向守当场咬掉(豁免一个没被拦的东西 = 僵尸)。 */
]
