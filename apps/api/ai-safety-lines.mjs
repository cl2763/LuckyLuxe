...
/* ⚠️ 裸 `余额` 曾在这里,05f 三跑时暴露:它把「储值卡**余额怎么查?**」也判成 3b ——
   而那是**问操作方法**,Cowork 勾定表里根本不在账户组(账户组只有 4 句)。
   按写死的判准「问规则怎么定的…这类一般问法 → answer」,它该答。
   **别人裁过的东西,抄就是抄** —— 我当时顺手多加了一句自己觉得该加的,产品和判据一起错。
   现在要么有第一人称所有格,要么是「剩多少/还有多少」这种**问结果**的形状。 */
const MY_ACCOUNT = /(我的|我卡|卡里|我还有|我有几|我攒|攒的)[^,,。;;]{0,6}(余额|次数|积分|点|券|卡|钱|钱数|多少)|(余额|积分|次数)[^,,。;;]{0,4}(还剩|剩多少|还有多少)|my (balance|points|credit|card)|my money|how much.*(left|on mine)/i
