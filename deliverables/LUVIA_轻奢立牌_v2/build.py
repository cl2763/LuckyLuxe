...
def section(s):
 n,en,rows,note=s
 return '<section><h2>'+n+' <small>'+en+'</small></h2><div class="prices-head"><span>项目</span><span>原价</span><span>分享价</span><span>会员价</span></div>'+''.join('<div class="row"><div>'+r[0]+('</div>' if not r[1] else '<small>'+r[1]+'</small></div>')+''.join('<span>'+str(v)+'</span>' for v in r[2:])+'</div>' for r in rows)+('<p class="note">'+note+'</p>' if note else '')+'</section>'
benefits=[
['全店项目享 <b>会员价</b>（原价 8 折）',('会员专属服务包','200'),'同行好友 85 折券 ×1（护理不适用）'],
['全店项目享 <b>金卡会员价</b>（原价 7.5 折）',('会员专属服务包','200'),'每年生日半价礼 ×1 · <b>金卡起专属</b>','同行好友 85 折券 ×1（护理不适用）',('富勒烯深层滋养手部护理 ×1','500'),'预约免定金 · <b>金卡起专属</b>'],
['全店项目享 <b>铂金卡会员价</b>（原价 7 折）',('会员专属服务包','200'),'每年生日半价礼 ×1','同行好友 85 折券 ×1（护理不适用）',('富勒烯深层滋养手部护理 ×1','500'),'预约免定金',('一对一单根嫁接睫毛体验卡 ×1','400'),'指定技师'],
['全店项目享 <b>黑卡会员价</b>（原价 6.5 折 · 全店最优）',('会员专属服务包','200'),'每年生日半价礼 ×1','同行好友 85 折券 ×1（护理不适用）',('富勒烯深层滋养手部护理 ×1','500'),('云足焕肤 SPA ×1','600'),('款式美睫体验卡 ×1','700'),'指定技师 · 优先预约时段']]
tiers=[('银卡','SILVER','8.0','即价目表「会员价」','充值 800 · 或累计消费 800','200'),('金卡','GOLD','7.5','于门店原价','累计充值 2,000 · 或累计消费 3,000','700'),('铂金卡','PLATINUM','7.0','于门店原价','累计充值 3,500 · 或累计消费 8,000','1,100'),('至尊黑卡','BLACK','6.5','于门店原价 · 全店最优','累计充值 5,000 · 或累计消费 15,000','2,000')]
...
for ix in [[0,1,2],[3,4,5],[6,7]]:html+='<div class="col">'+''.join(section(sections[i]) for i in ix)+'</div>'
...
html+='</div><div class="example"><span>以「精品单色 <s>¥360</s>」为例</span><span>银卡 <b>288</b></span><span>金卡 <b>270</b></span><span>铂金卡 <b>252</b></span><span>至尊黑卡 <b>234</b></span></div><div class="rules"><p><b>会员规则</b>　累计消费与充值双轨判定，按较高等级享受权益，等级永久有效。赠礼券于开卡后发放至小程序券包，自开卡日起 12 个月内有效，需提前预约，不折现、不转让。生日礼以系统登记生日为准，首次开卡当月不计入。卡内余额上限 5,000。各等级均直接按门店原价打折结算（银卡 8 折即价目表会员价，金卡 7.5 折，铂金卡 7 折，至尊黑卡 6.5 折），四舍五入至元；卸甲类为工本价，不叠加等级折扣。</p><p>退卡方式：退卡金额 = 卡内总金额 − 已消费项目按门店原价（非会员价）折算，多退少补。请谨慎储值，一经办卡即视为同意以上规则。</p></div><div class="appointment">BY APPOINTMENT ONLY</div></main>'
