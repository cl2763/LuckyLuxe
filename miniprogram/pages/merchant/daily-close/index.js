/* 屏 1(下半)日结 + 屏 1b 金额更正 · 小程序版
   金额红线:本页不算钱。分成比例是店长填的输入,金额一律由后端算好回传;
...
Page({
  ...
  async onShow() {
    ...
    this.load(this.data.date || storeToday())
  },
  ...
  prevDay() { this.load(shiftDate(this.data.date, -1)) },
  nextDay() { this.load(shiftDate(this.data.date, 1)) },
  today() { this.load(storeToday()) },
  toggleOpen(e) {
